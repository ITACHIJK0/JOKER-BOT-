...
import fs from 'fs' // تم التأكد من استيراد fs لدعم فحص مسارات SubBot
...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        // 🛡️ فحص حظر الجروب والبوتات الفرعية (مع استثناء أوامر فك الحظر للمطور أو الاستجابة الذكية)
        ...
        // التحقق من أوامر فك الحظر للسماح لها بالمرور دائماً
        const isUnbanCommand = /(بانشاتفك|فك_بانشات|فك_الحظر|تفعيل|unbanchat|desbanearbot|اشتغل_هنا)/i.test(m.text || '')

        // 1. فحص حظر الجروب العام
        if (chatData.isBanned && !isUnbanCommand) {
            return // البوت موقوف كلياً في هذا الجروب ولا يستجيب إلا للفك
        }

        // 2. فحص حظر البوت الفرعي (SubBot) الخاص بالمستخدم في هذا الجروب
        if (chatData.subBotsBanned && chatData.subBotsBanned[botPath] && isSocketUser && !isROwner && !isUnbanCommand) {
            return // هذا البوت الفرعي موقوف في هذا الجروب
        }
        ...
        try {
            ...
            if (chat) {
                ...
                if (!('subBotsBanned' in chat)) chat.subBotsBanned = {}
