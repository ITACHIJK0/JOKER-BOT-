// plugins/q5-download.js
// ✧ THE JOKER & ITACHI - Download Menu 📥
...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    // التفاعل بإيموجي القسم الخاص بالتحميل
    await conn.sendMessage(m.chat, { react: { text: '📥', key: m.key } });
    const imageUrl = 'https://i.postimg.cc/qBQdtfKs/29e9694da6f20598025cdd74ef0dd56e.jpg';
...
❖ ── ✦ ── ❖ ── ✦ ── ❖ ── ✦ ── ❖
 ...
 ┠ 📥 ╎ قــسم الـتَّـحـمِـيــل والمِيدْيَا
 ┠ ${_p}apk
 ┠ ${_p}انستا
 ┠ ${_p}فيس
 ┠ ${_p}تيك
...
> 亗 𝐉𝐨𝐤𝐞𝐫 𝐁𝐨𝐭 ✰ 𝐁𝐲 𝐈𝐭𝐚𝐜𝐡𝐢 ♞`;
    ...
    const nativeFlowPayload = {
      ...
      header: {
        ...
        subtitle: '📥 قـسـم التَّحميل والمِيدْيَا',
    ...
    console.error('[Joker-Download]', e);
    await conn.sendMessage(m.chat, {
      text: theme.build([
        { type: 'title', text: '🃏 الـجـوكـر: "خطأ"' },
        { type: 'warning', text: 'حدث خطأ أثناء تحميل لوحة التحميل' }
      ])
    }, { quoted: m });
  }
}
...
async function makeFkontak() {
  try {
    const res = await fetch('https://i.postimg.cc/qBQdtfKs/29e9694da6f20598025cdd74ef0dd56e.jpg');
...
handler.help = ['ق5', 'التحميل', 'الميديا'];
handler.tags = ['main'];
handler.command = /^(ق5|التحميل|الميديا)$/i;
