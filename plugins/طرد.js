...
let handler = async (m, { conn, participants, isAdmin, isBotAdmin, text }) => {
    ...
    const isDeveloper = allowedOwners.some(dev => {
        ...
        let targetClean = target.replace(/[^0-9]/g, '');
    ...
    let matched = participants.find(p =>
    ...
    let stillThere = updatedMeta.participants.find(p =>
        p.id === matched.id || p.phoneNumber === matched.phoneNumber
    );
    ...
    if (stillThere) {
        ...
        const successText = theme.build([
            ...
            { type: 'divider' },
            { type: 'info', label: '🎯 الهدف المنفي', value: '@' + targetNumber },
        ...
        return await conn.relayMessage(m.chat, msg.message, { messageId: m.key.id });
    }
};
