...
const maleNames = [
    ...
    'تامر', 'باسم', 'سعيد', 'جمال', 'سامي', 'وائل', 'أسامة', 'معاذ', 'يزيد', 'الأتاتشي', 'الجوكر', 'احمد'
];
    ...
    try {
        ...
        if (ps.length < 2) {
            return m.reply(theme.build([
                ...
                { type: 'warning', text: 'المجموعة تحتاج إلى عضوين على الأقل لتنفيذ المراسم' },
                { type: 'divider' },
                { type: 'line', text: '👑 𝐈𝐭𝐚𝐜𝐡𝐢♞ | 𝐓𝐇𝐄 𝑱𝑶𝑲𝑬𝑹 ᜰ' }
            ]));
        }

        // أرقام المطور المعتمدة
        const ownerNumbers = ['249927142037@s.whatsapp.net', '249916221538@s.whatsapp.net'];
        if (global.owner) {
            global.owner.forEach(o => {
                if (o && o[0]) ownerNumbers.push(o[0] + '@s.whatsapp.net');
            });
        }
        ...
        if (command === 'طلاق' || command === 'اتطلقوا' || command === 'اطلاق') {
            imageUrl = 'https://i.postimg.cc/kG7t6RgX/274d9fa742b3a4e961e8a643217a4b06.jpg';
            
            // اختيار شخصين عشوائيين للطلاق السريع في المجموعة
            let target1 = ps[Math.floor(Math.random() * ps.length)];
            let target2;
            do {
            ...
            let captionText = theme.build([
                { type: 'title', text: '💔 𝐈𝐭𝐚𝐜𝐡𝐢: "محكمة البوت - إعلان طلاق رسمي"' },
                { type: 'divider' },
                { type: 'info', label: '👤 الطرف الأول', value: toM(target1) },
                { type: 'info', label: '👤 الطرف الثاني', value: toM(target2) },
                { type: 'divider' },
                { type: 'line', text: '⚖️ بعد مداولة القضاة واستحالة العشرة، تم الانفصال بالثلاثة!' },
                { type: 'line', text: '🚪 كل واحد يروح في طريق.. انتهت القصة الخيالية.' },
                { type: 'divider' },
                { type: 'line', text: '👑 𝐈𝐭𝐚𝐜𝐡𝐢♞ | 𝐓𝐇𝐄 𝑱𝑶𝑲𝑬𝑹 ᜰ' }
            ]);
        ...
        // 💍 معالجة أوامر الزواج (زواج / زوجني / تزوج)
        if (command === 'زواج') {
            if (isOwnerTrigger) {
        ...
        await m.reply(theme.build([
            { type: 'title', text: '❄️ 𝐈𝐭𝐚𝐜𝐡𝐢: "فشلت العملية"' },
            { type: 'subtitle', text: err.message || 'خطأ غير معروف في معالجة الطلب' },
            { type: 'divider' },
            { type: 'line', text: '👑 𝐈𝐭𝐚𝐜𝐡𝐢♞ | 𝐓𝐇𝐄 𝑱𝑶𝑲𝑬𝑹 ᜰ' }
        ]));
    }
...
handler.help = ['زوجني', 'زواج', 'طلاق'];
handler.tags = ['entertainment'];
handler.command = /^(زوجني|زواج|تزوج|طلاق|اتطلقوا|اطلاق)$/i;
handler.group = true;
