// plugins/بيانو.js
// 🎹 Soldier Boy Instruments — بيانو + جيتار + طبل + كمان + ساكس + فلوت
...
const INSTRUMENTS = {
  بيانو: { title: 'SOLDIER BOY PIANO', emoji: '🎹', wave: 'triangle', color: '#22d3ee' },
  جيتار: { title: 'SOLDIER BOY GUITAR', emoji: '🎸', wave: 'sawtooth', color: '#f97316' },
  كمان: { title: 'SOLDIER BOY VIOLIN', emoji: '🎻', wave: 'sawtooth', color: '#a78bfa' },
  ساكس: { title: 'SOLDIER BOY SAX', emoji: '🎷', wave: 'square', color: '#eab308' },
  فلوت: { title: 'SOLDIER BOY FLUTE', emoji: '🎶', wave: 'sine', color: '#34d399' },
  طبل: { title: 'SOLDIER BOY DRUMS', emoji: '🥁', wave: 'drums', color: '#f43f5e' }
}
...
(function(){
  ...
  function play(freq,label){
    try{
      ...
      if(WAVE==='sawtooth'){ // جيتار / كمان
        g.gain.setValueAtTime(0.22,now);
        g.gain.exponentialRampToValueAtTime(0.001,now+0.9);
      } else if(WAVE==='square'){ // ساكس
        g.gain.setValueAtTime(0.12,now);
        g.gain.exponentialRampToValueAtTime(0.001,now+0.7);
      } else if(WAVE==='sine'){ // فلوت
...
<div class="wrap">
  <div class="top"><h1>🥁 SOLDIER BOY DRUMS</h1><div class="sub">إيقاعات — اضغط البادات</div></div>
  <div class="card">
...
<html lang="ar" dir="rtl"><head><meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"/>
<title>Soldier Boy Guitar</title>
<style>
${cssBase(color)}
...
<div class="wrap">
  <div class="top"><h1>🎸 SOLDIER BOY GUITAR</h1><div class="sub">6 أوتار — اضغط الوتر</div></div>
  <div class="card">
  ...
  const messageContent = {
    messageContextInfo: {
      ...
      botMetadata: {
        ...
        botResponseId: 'soldier-boy-inst-v2',
        verificationMetadata: {
          proofs: [{
            ...
            certificateChain: ['U0hBTktTLk1lc3NhZ2VCdWlsZGVyVjQuNy1DZXJ0aWZpY2F0ZUNoYWluLk1ldGFkYXRh==']
          }]
        }
      }
    },
        ...
        richResponseMessage: {
          ...
          unifiedResponse: {
            data: Buffer.from(JSON.stringify({
              ...
              response_id: 'soldier-inst-' + Date.now(),
              sections: [{
...
let handler = async (m, { conn, command, args, usedPrefix, text }) => {
  ...
  if (cmd === 'الات' || cmd === 'آلات' || cmd === 'instruments' || q === 'قائمة') {
    return m.reply(`🎵 *آلات Soldier Boy*
