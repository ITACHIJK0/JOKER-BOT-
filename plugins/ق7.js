// plugins/q7-manga.js
// ✧ THE JOKER & ITACHI - Manga & Anime Menu 📚
...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    // التفاعل بإيموجي القسم الخاص بالمانجا
    await conn.sendMessage(m.chat, { react: { text: '📚', key: m.key } });

    const imageUrl = 'https://i.postimg.cc/MpVRDYqf/517150a350065a2584711351673f46d4.jpg';
...
❖ ── ✦ ── ❖ ── ✦ ── ❖ ── ✦ ── ❖
 ...
 ┠ 📚 ╎ قــسم الـمَـانـجـا والأنْـمِـي
 ┠ ${_p}انمي
 ┠ ${_p}مانجا_توك
 ┠ ${_p}مانهوا
 ┠ ${_p}انمي_ايديت
...
> 亗 𝐉𝐨𝐤𝐞𝐫 𝐁𝐨𝐭 ✰ 𝐁𝐲 𝐈𝐭𝐚𝐜𝐡𝐢 ♞`;
    ...
    const nativeFlowPayload = {
      ...
      header: {
        ...
        subtitle: '📚 قـسـم المَانجَا والأنمِي',
    ...
    console.error('[Joker-Manga]', e);
    await conn.sendMessage(m.chat, {
      text: theme.build([
        { type: 'title', text: '🃏 الـجـوكـر: "خطأ"' },
        { type: 'warning', text: 'حدث خطأ أثناء تحميل لوحة المانجا' }
      ])
    }, { quoted: m });
  }
}
...
async function makeFkontak() {
  try {
    const res = await fetch('https://i.postimg.cc/MpVRDYqf/517150a350065a2584711351673f46d4.jpg');
...
handler.help = ['ق7', 'المانجا'];
handler.tags = ['main'];
handler.command = /^(ق7|المانجا)$/i;
