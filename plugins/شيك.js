...
const sendThemedReply = async (conn, m, title, lines) => {
  const content = [
    { type: 'title', text: title },
    { type: 'divider' }
  ];
  lines.forEach(line => {
    content.push({ type: 'line', text: line });
  });
  content.push({ type: 'divider' });
  content.push({ type: 'line', text: '⌁ 𝙹𝙾𝙺𝙴𝚁 𝐗 𝙸𝚃𝙰𝙲𝙷𝙸 ⌁' });
...
let handler = async (m, { conn, text }) => {
  ...
  if (!phone) {
    return sendThemedReply(conn, m, '⚠️ خطأ في الاستخدام', ['ما لقيت رقم هاتف صالح.']);
  }
  ...
  try {
    ...
    if (!onWa || !onWa.exists) {
      return sendThemedReply(conn, m, '🔍 فحص الرقم', [
        `*الرقم:* ${phone}`,
    ...
    if (response?.reason === "blocked" || response?.in_app_ban_appeal === 1) {
      ...
      if (response.appeal_token) {
        lines.push(`*appeal_token:* \`${response.appeal_token}\``);
      }

      return sendThemedReply(conn, m, '🚨 تقرير حظر الرقم', lines);
    } else {
      const lines = [
        `*الرقم:* ${phone}`,
      ...
      if (response.fallback_methods?.length) {
        lines.push(`*طرق التحقق:* ${response.fallback_methods.join(", ")}`);
      }

      return sendThemedReply(conn, m, '🟢 تقرير فحص الرقم', lines);
    }
  } catch (e) {
    return sendThemedReply(conn, m, '❌ خطأ تقني', [`خطأ: ${e.message}`]);
  }
};
