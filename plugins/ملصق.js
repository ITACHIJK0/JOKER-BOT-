// plugins/sticker.js
// 👑 𝐈𝐭𝐚𝐜𝐡𝐢♞ | 𝐓𝐇𝐄 𝑱𝑶𝑲𝑬𝑹 ᜰ - Advanced Smart Multi-Media Sticker & Pack Maker (FFmpeg Edition) 🎨⚔️
...
async function addStickerMetadata(webpBuffer, packName = '👑 𝐈𝐭𝐚𝐜𝐡𝐢 𝑷𝒂𝒄𝒌 ✧', authorName = '👑 𝐈𝐭𝐚𝐜𝐡𝐢♞ | 𝐓𝐇𝐄 𝑱𝑶𝑲𝑬𝑹 𝜰') {
  try {
...
// 📦 وظائف دعم حزمة الملصقات المتقدمة (WhatsApp Sticker Pack)
...
async function sendStickerPack(conn, m, pack, packTitle = '👑 𝐈𝐭𝐚𝐜𝐡𝐢 𝑷𝒂𝒄𝒌 ✧') {
  ...
  await conn.relayMessage(m.chat, {
    ...
    stickerPackMessage: {
      ...
      name: packTitle,
      publisher: '👑 𝐈𝐭𝐚𝐜𝐡𝐢♞ | 𝐓𝐇𝐄 𝑱𝑶𝑲𝑬𝑹 𝜰',
      packDescription: '👑 𝐈𝐭𝐚𝐜𝐡𝐢 & 𝑱𝑶𝑲𝑬𝑹 Official Sticker Pack',
...
let handler = async (m, { conn }) => {
    // التحقق من الرسالة المقتبسة أو الوسائط المرسلة مباشرة مع الأمر
    let q = m.quoted ? m.quoted : m
    let mime = (q.msg || q).mimetype || ''

    // دعم استخراج الوسائط المتعددة (المرسلة كألبوم أو رسائل متعددة مرفقة)
    let mediaMessages = []
    
    // التحقق إذا كانت الرسالة المقتبسة عبارة عن ألبوم أو تحتوي على رسالة مرفقة متعددة
    if (m.quoted) {
        if (m.quoted.message?.imageMessage || m.quoted.message?.videoMessage || m.quoted.message?.documentMessage) {
            mediaMessages.push(m.quoted)
        }
    }
    
    // التحقق من الرسالة الحالية نفسها
    if (mime || q.message) {
        mediaMessages.push(q)
    }

    // فحص رسائل الألبوم المضمنة في Baileys الحديثة (extendedMessage أو message context)
    let quotedMsg = m.quoted?.message
    if (quotedMsg) {
        if (quotedMsg.viewOnceMessageV2?.message?.imageMessage || quotedMsg.viewOnceMessageV2?.message?.videoMessage) {
            mediaMessages.push({ quoted: true, msg: quotedMsg.viewOnceMessageV2.message, download: async () => await m.quoted.download() })
        }
    }

    // إزالة التكرار إن وجد
    mediaMessages = Array.from(new Set(mediaMessages))

    if (mediaMessages.length === 0 && !mime) {
        return m.reply('👑 *𝐈𝐭𝐚𝐜𝐡𝐢: "الرجاء الرد على صورة، فيديو، متحركة GIF، أو ألبوم صور/فيديوهات لتوليد الملصق أو الباكج"*')
    }

    await conn.sendMessage(m.chat, { react: { text: '⚡', key: m.key } })

    try {
        // إذا كان هناك أكثر من عنصر (ألبوم وسائط متعددة)، يتم إرسالها كحزمة (Sticker Pack) مطورة
        if (mediaMessages.length > 1 || (m.message && m.message.extendedTextMessage === false && m.quoted)) {
            // سنقوم بتحميل ومعالجة كافة العناصر كباكج
            let pack = []
            for (let i = 0; i < mediaMessages.length; i++) {
                let targetMsg = mediaMessages[i]
                let targetMime = (targetMsg.msg || targetMsg).mimetype || ''
                let isVideo = targetMime.includes('video') || targetMime.includes('gif')
                let buffer = await targetMsg.download()

                let inputPath = join(tmpdir(), `pk_in_${Date.now()}_${i}.${isVideo ? 'mp4' : 'jpg'}`)
                let outputPath = join(tmpdir(), `pk_out_${Date.now()}_${i}.webp`)

                writeFileSync(inputPath, buffer)
                try {
                    if (!isVideo) {
                        let cmd = `ffmpeg -y -i "${inputPath}" -vf "scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000" -vcodec libwebp -lossless 0 -q:v 85 "${outputPath}"`
                        await execAsync(cmd, { timeout: 30000 })
                    } else {
                        let cmd = `ffmpeg -y -i "${inputPath}" -t 7 -vf "fps=15,scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000" -c:v libwebp -loop 0 -preset default -an -vsync 0 "${outputPath}"`
            ...
            if (pack.length > 0) {
                await sendStickerPack(conn, m, pack, '👑 𝐈𝐭𝐚𝐜𝐡𝐢 & 𝑱𝑶𝑲𝑬𝑹 Pack')
                await conn.sendMessage(m.chat, { react: { text: '✅', key: m.key } })
                return
            }
        }

        // معالجة عنصر فردي (صورة، فيديو، GIF، أو وسائط منفردة)
        ...
        try {
            if (!isVideo) {
                // 🖼️ معالجة الصورة الفردية بدقة تامة وبدون قص أطراف
                let cmd = `ffmpeg -y -i "${inputPath}" -vf "scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000" -vcodec libwebp -lossless 0 -q:v 85 "${outputPath}"`
                await execAsync(cmd, { timeout: 30000 })
            } else {
                // 🎞️ معالجة الفيديو أو الـ GIF المتحرك (أول 7 ثوانٍ)
                let cmd = `ffmpeg -y -i "${inputPath}" -t 7 -vf "fps=15,scale=512:512:force_original_aspect_ratio=decrease,pad=512:512:(ow-iw)/2:(oh-ih)/2:color=0x00000000" -c:v libwebp -loop 0 -preset default -an -vsync 0 "${outputPath}"`
                await execAsync(cmd, { timeout: 45000 })
            }
            ...
            if (existsSync(outputPath)) {
                ...
                // إرسال الملصق الفردي بحقوق أتاتشي والجوكر
