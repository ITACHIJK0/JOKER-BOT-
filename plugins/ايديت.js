...
// ✧ 𝐈𝐭𝐚𝐜𝐡𝐢♞ | 𝐓𝐇𝐄 𝑱𝑶𝑲𝑬𝑹 ᜰ — نظام جلب الإيديتات الأسطوري (Multi-Source Engine) 🎬⚔️
  ...
  for (const q of queries) {
    try {
      ...
      const videos = (result?.videos || [])
        ...
        .filter(v => v.seconds > 0 && v.seconds <= 300) // فيديوهات تحت 5 دقائق
...
// 🛡️ نظام تحميل متعدد المصادر للإيديتات لضمان عدم تلف أو فشل الملفات أبداً
class MultiSourceEditDownloader {
  // المصدر الأول: Cobalt API (أسرع وأقوى سيرفر مباشر للإيديتات والفيديوهات القصيرة)
  async downloadFromCobalt(url, outputPath) {
    try {
      ...
      if (response.data && (response.data.status === 'stream' || response.data.status === 'redirect')) {
        const videoStreamRes = await axios.get(response.data.url, {
          responseType: 'arraybuffer',
          timeout: 60000,
          maxContentLength: Infinity
        })
        const { writeFileSync } = await import('fs')
        writeFileSync(outputPath, videoStreamRes.data)
        return true
      }
    } catch (e) {
      console.log('[Cobalt-Edit-Fallback] Trying next engine...')
    }
    throw new Error('فشل مصدر Cobalt')
  }

  // المصدر الثاني: yt-dlp الأساسي
  async downloadFromYtDlp(url, outputPath) {
    await getYTDLP()
  ...
  async download(url, outputPath) {
    try {
      return await this.downloadFromCobalt(url, outputPath)
    } catch (e1) {
      return await this.downloadFromYtDlp(url, outputPath)
    }
  }
}
...
〽️ 𝐉𝐎𝐊𝐄𝐑 𝐁𝐎𝐓 ♞ 𝐁𝐘 𝐈𝐓𝐀𝐂𝐇𝐈 卍`;
  ...
  try {
    ...
    let lastError = null
    // تجربة أول 6 فيديوهات متاحة لضمان النجاح الفوري
