...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    // التفاعل بإيموجي القسم الخاص بالألعاب
    await conn.sendMessage(m.chat, { react: { text: '🎮', key: m.key } });

    const imageUrl = 'https://i.postimg.cc/BvNCTCBm/9a46ea80051d1db74c2675c300a7014e.jpg';
...
❖ ── ✦ ── ❖ ── ✦ ── ❖ ── ✦ ── ❖
 ...
 ┠ 🎮 ╎ قــسم الألــعــاب والتَّسلِيَة
 ┠ ${_p}عاصمه
 ┠ ${_p}ايموجي
 ┠ ${_p}كت
 ┠ ${_p}سؤال
...
> 亗 𝐉𝐨𝐤𝐞𝐫 𝐁𝐨𝐭 ✰ 𝐁𝐲 𝐈𝐭𝐚𝐜𝐡𝐢 ♞`;
    ...
    const nativeFlowPayload = {
      ...
      header: {
        ...
        subtitle: '🎮 قـسـم الألـعـاب والتَّسلِيَة',
    ...
    console.error('[Joker-Games]', e);
    await conn.sendMessage(m.chat, {
      text: theme.build([
        { type: 'title', text: '🃏 الـجـوكـر: "خطأ"' },
        { type: 'warning', text: 'حدث خطأ أثناء تحميل لوحة الألعاب' }
      ])
    }, { quoted: m });
  }
}
...
async function makeFkontak() {
  try {
    const res = await fetch('https://i.postimg.cc/BvNCTCBm/9a46ea80051d1db74c2675c300a7014e.jpg');
...
handler.help = ['ق4', 'الالعاب'];
handler.tags = ['main'];
handler.command = /^(ق4|الالعاب)$/i;
