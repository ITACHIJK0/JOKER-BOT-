...
let timeout = 60000
let poin = 50 // تم ضبط الجائزة لتكون 50 نقطة وذهب بناءً على طلبك

// دالة لقراءة قاعدة البيانات الموحدة للمجموعة وتجنب أي خطأ
...
handler.before = async (m, { conn, usedPrefix }) => {
  ...
  if (gameType === 'فكك') {
    ...
    // في حالة فكك، يقبل العضو الإجابة سواء كانت مفرقة بالأحرف أو متصلة طالما الحروف صحيحة
    if (userClean === correctClean || userAnswer.includes(correctClean)) {
      isMatch = true
    }
  } else {
    // في حالة كتابه، يجب أن تكون الكلمة متصلة وصحيحة
    let userClean = userAnswer.replace(/\s+/g, '')
    let correctClean = correctAnswer.replace(/\s+/g, '')
  ...
  if (isMatch) {
    ...
    // إذا لم يكن العضو مسجلاً في بنك المجموعة، افتح له حساباً جديداً تلقائياً
    if (!db[m.sender]) {
      db[m.sender] = {
    ...
    // إضافة الجائزة (50 نقطة وذهب) إلى البنك
