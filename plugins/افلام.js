...
const handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        if (command === 'فيلم' || command === 'movie') {
            ...
            const sections = [{
                ...
                rows: results.slice(0, 10).map(item => ({
                    title: (item.title || 'بدون عنوان').substring(0, 30),
                    description: `⭐ ${item.rating || 'N/A'} | 🎭 ${item.type || 'غير معروف'}`,
                    id: `${usedPrefix}حلقات ${item.id}|${item.typeId}|${encodeURIComponent(item.title || 'عرض')}`
                }))
            }]
        ...
        if (command === 'حلقات') {
            ...
            const title = decodeURIComponent(rawTitle || 'عرض')
            ...
            const sections = [{
                ...
                rows: episodes.slice(0, 25).map(ep => ({
        ...
        if (command === 'تحميل') {
            ...
            const title = decodeURIComponent(rawTitle || 'فيلم')
            
            await sendThemedText(conn, m, '🚀 جاري التحميل', `جاري جلب وتحميل ${title} (الحلقة ${episode})... يرجى الانتظار قد يستغرق بعض الوقت.`)

            const downloadData = await downloadEpisode(subjectId, subjectType, episode)

            if (downloadData.url) {
                try {
                    // محاولة الإرسال المباشر كفيديو
                    ...
                    // إذا فشل كفيديو (لحجمه أو ضغطه)، يتم إرساله فوراً كملف وثيقة (Document) بدون أي تلف أو فقدان للجودة
                    await conn.sendMessage(m.chat, {
                        document: { url: downloadData.url },
                        mimetype: 'video/mp4',
                        fileName: `${title.replace(/[\/\\?%*:|"<>]/g, '')}_EP${episode}.mp4`,
                        caption: `✨ ━━━〔 *تم التحميل كملف عالي الجودة* 🎬 〕━━━ ✨\n\n📺 ┊ *العنوان:* ${title}\n📌 ┊ *الحلقة:* ${episode}\n\n⌁ 𝙹𝙾𝙺𝙴𝚁 𝐗 𝙸𝚃𝙰𝙲𝙷𝙸 ⌁`
                    }, { quoted: m })
                }
                await m.react('✅')
            } else {
                throw new Error('فشل الحصول على رابط التحميل المباشر من السيرفر!')
            }
        }
