// plugins/q1-islamic.js
// ✧ THE JOKER & ITACHI - Islamic Menu 🕌
...
let handler = async (m, { conn, usedPrefix: _p }) => {
  try {
    ...
    // التفاعل بإيموجي القسم الخاص بالدين
...
❖ ── ✦ ── ❖ ── ✦ ── ❖ ── ✦ ── ❖
 ...
 ┠ 🕌 ╎ قــسم الـديـن والـقـرآن الكـريـم
 ┠ ${_p}قران
 ┠ ${_p}ايه
 ┠ ${_p}حديث
 ┠ ${_p}دعاء
...
> 亗 𝐉𝐨𝐤𝐞𝐫 𝐁𝐨𝐭 ✰ 𝐁𝐲 𝐈𝐭𝐚𝐜𝐡𝐢 ♞`;
    ...
    const nativeFlowPayload = {
      ...
      header: {
        ...
        subtitle: '🕌 قـسـم الـديـن والـقـرآن',
    ...
    console.error('[Joker-Islamic]', e);
    await conn.sendMessage(m.chat, {
      text: theme.build([
        { type: 'title', text: '🃏 الـجـوكـر: "خطأ"' },
        { type: 'warning', text: 'حدث خطأ أثناء تحميل لوحة الدين' }
      ])
    }, { quoted: m });
  }
}
...
handler.help = ['ق1', 'الدين'];
handler.tags = ['main'];
handler.command = /^(ق1|الدين)$/i;
