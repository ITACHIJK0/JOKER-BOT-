...
// ✧ 𝐉𝐎𝐊𝐄𝐑 𝐁𝐎𝐓 - حزمة ملصقات Pinterest 📦
...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  let statusMsg = await m.reply(`🃏 *𝐉𝐎𝐊𝐄𝐑 𝐁𝐎𝐓* جاري البحث عن "${query}"... ⏳`)
  ...
  try {
    ...
    if (pack.length === 0) {
      return m.reply(`❌ *𝐉𝐎𝐊𝐄𝐑 𝐁𝐎𝐓* ➢ تعذر معالجة أي ملصق`)
    }
    ...
    console.error(e)
    try { await conn.sendMessage(m.chat, { delete: statusMsg.key }) } catch {}
    m.reply(`❌ *𝐉𝐎𝐊𝐄𝐑 𝐁𝐎𝐓* ➢ خطأ: ${e.message}`)
  }
}
