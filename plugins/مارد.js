...
async function createAki() {
  try {
    ...
    if (!Aki) throw new Error('مكتبة aki-api غير مُصدرة بشكل صحيح')
    const aki = new Aki({ region: 'ar', childMode: false })
    await aki.start()
    return { type: 'aki-api', aki }
  } catch (e) {
    throw new Error(`مارد API غير متاح: ${e?.message || e}`)
  }
}
...
async function stepAnswer(session, answerIndex) {
  await session.aki.step(answerIndex)
}

async function stepBack(session) {
  if (typeof session.aki.back === 'function') {
    await session.aki.back()
  } else {
    throw new Error('رجوع غير مدعوم')
  }
}
...
function guessData(session) {
  const a = session.aki
  return {
    name: a.answers?.[0]?.name || a.guess?.name || 'شخصية غير معروفة',
    desc: a.answers?.[0]?.description || a.guess?.description || '',
    img: a.answers?.[0]?.absolute_picture_path || a.guess?.absolute_picture_path || null
  }
}
...
اضغط زر الإجابة:`
  ...
  const nativeFlowPayload = {
    body: { text: body },
    footer: { text: '🃏 𝐉𝐎𝐊𝐄𝐑 𝐁𝐎𝐓 • المطور: 𝐈𝐭𝐚𝐜𝐡𝐢' },
    nativeFlowMessage: {
      buttons: [
        {
          name: 'single_select',
...
لعبة جديدة: ${usedPrefix}مارد`
      ...
      sessions.delete(key)
      return m.reply(`❌ حصل خطأ أثناء اللعب: ${e.message || e}\nابدأ من جديد: ${usedPrefix}مارد`)
    }
  }
